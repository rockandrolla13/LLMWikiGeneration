---
title: Chain-of-Thought Prompting
page_id: concepts/chain-of-thought-prompting
page_type: concept
revision_id: 1
created: '2026-05-17T16:44:06Z'
updated: '2026-05-17T16:44:06Z'
updated_by: wiki-batch-ai-engineering-2026-05-17
tags:
- ai-engineering
- in-context-learning
- prompt-engineering
sources:
- sources/alammar-2024-hands-on-llm
- sources/berryman-2024-prompt-engineering-llms
- sources/boonstra-2024-google-prompt-engineering
- sources/caelen-2023-developing-apps-gpt4
- sources/oshin-2025-learning-langchain
- sources/taulli-2024-ai-assisted-programming
related:
- concepts/in-context-learning
- concepts/prompt-engineering
mind_map_priority: high
---

# Chain-of-Thought Prompting

## Definition

A prompting technique introduced by Google researchers (Wei et al., 2022) that improves LLM reasoning by decomposing a problem into intermediate steps, used in coding to walk a model through requirement, design, and implementation actions sequentially.

## Sources

- [[sources/alammar-2024-hands-on-llm|Hands-On Large Language Models]]
- [[sources/berryman-2024-prompt-engineering-llms|Prompt Engineering for LLMs]]
- [[sources/boonstra-2024-google-prompt-engineering|Prompt Engineering]]
- [[sources/caelen-2023-developing-apps-gpt4|Developing Apps with GPT-4 and ChatGPT]]
- [[sources/oshin-2025-learning-langchain|Learning LangChain]]
- [[sources/taulli-2024-ai-assisted-programming|AI-Assisted Programming]]

## Related Concepts

- [[concepts/in-context-learning]]
- [[concepts/prompt-engineering]]
