---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/heterogeneous-treatment-effects
page_type: concept
related:
- concepts/causal-inference
- concepts/potential-outcomes
revision_id: 1
sources:
- sources/hill-2011-bart-causal-inference
tags: []
title: Heterogeneous Treatment Effects
updated: '2026-06-20T01:03:51Z'
updated_by: creditmacro-batch
schema_version: 2
uuid: e483f1f5-0ab8-5db2-aa52-7cf9b1f68607
content_hash: sha256:de9ba2fa44d9382d7350f63d6da34a6541276c8ff46cbdacc5d3505a2b3fe2aa
---

<!-- AUTHORED REGION START -->
# Heterogeneous Treatment Effects

## Definition

Variation in the causal treatment effect across units or subgroups defined by covariates, which BART captures automatically through tree-based interactions rather than pre-specified interaction terms.

## Sources

- [[sources/hill-2011-bart-causal-inference|Bayesian Nonparametric Modeling for Causal Inference]]

## Related Concepts

- [[concepts/causal-inference|causal-inference]]
- [[concepts/potential-outcomes|potential-outcomes]]
<!-- AUTHORED REGION END -->
