---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/futures-carry
page_type: concept
related:
- concepts/carry-rolldown
revision_id: 1
sources:
- sources/carver-2023-advanced-futures-trading-strategies
tags: []
title: Futures Carry
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Futures Carry

## Definition

A strategy exploiting the shape of the futures term structure to predict and harvest expected returns, going long high-carry and short low-carry instruments.

## Sources

- [[sources/carver-2023-advanced-futures-trading-strategies|Advanced Futures Trading Strategies]]

## Related Concepts

- [[concepts/carry-rolldown|carry-rolldown]]