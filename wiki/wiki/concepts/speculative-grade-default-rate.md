---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/speculative-grade-default-rate
page_type: concept
related:
- concepts/corporate-bonds
- concepts/credit-spread-curve
- concepts/distressed-exchange
revision_id: 1
sources:
- sources/caprio-2026-steady-but-ai
tags: []
title: Speculative-Grade Default Rate
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Speculative-Grade Default Rate

## Definition

The trailing rate at which speculative-grade (high-yield and leveraged-loan) issuers default, including distressed exchanges; tracked for US and Europe across B2/B3/CCC cohorts.

## Sources

- [[sources/caprio-2026-steady-but-ai|2026: Steady, but AI & the Hawks are Circling (Software)]]

## Related Concepts

- [[concepts/corporate-bonds|corporate-bonds]]
- [[concepts/credit-spread-curve|credit-spread-curve]]
- [[concepts/distressed-exchange|distressed-exchange]]