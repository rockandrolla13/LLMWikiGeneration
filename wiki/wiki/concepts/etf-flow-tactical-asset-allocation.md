---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/etf-flow-tactical-asset-allocation
page_type: concept
related:
- concepts/etf-flows
- concepts/factor-investing
- concepts/market-timing
revision_id: 1
sources:
- sources/mercado-2015-taarss-flow-whisperer
tags: []
title: ETF Flow-Based Tactical Asset Allocation
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# ETF Flow-Based Tactical Asset Allocation

## Definition

A tactical allocation approach inferring investor asset-allocation preferences from ETF/ETV cumulative cash-flow trends, used to rotate across asset classes, regions, sectors, and duration buckets.

## Sources

- [[sources/mercado-2015-taarss-flow-whisperer|The Flow Whisperer: TAARSS says prefer a mix of bonds and equities in Q1]]

## Related Concepts

- [[concepts/etf-flows|etf-flows]]
- [[concepts/factor-investing|factor-investing]]
- [[concepts/market-timing|market-timing]]