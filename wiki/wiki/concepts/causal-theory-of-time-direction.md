---
abstraction_level: intermediate
concept_type: theory
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/causal-theory-of-time-direction
page_type: concept
related:
- concepts/causal-inference
revision_id: 1
sources:
- sources/tooley-1997-time-tense-causation
tags: []
title: Causal Theory of the Direction of Time
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Causal Theory of the Direction of Time

## Definition

The view that the direction and order of time is explained in terms of the direction of causation, so temporal priority is analyzed via the asymmetry that causes are not later than their effects.

## Sources

- [[sources/tooley-1997-time-tense-causation|Time, Tense, and Causation]]

## Related Concepts

- [[concepts/causal-inference|causal-inference]]