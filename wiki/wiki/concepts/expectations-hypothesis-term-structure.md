---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/expectations-hypothesis-term-structure
page_type: concept
related:
- concepts/factor-models
- concepts/forward-rate
- concepts/government-bond-spreads
- concepts/mean-reversion
- concepts/term-structure-risk-premium
revision_id: 2
sources:
- sources/bams-2003-risk-premia-term-structure-panel
- sources/ms-2013-12-13-top-10-rates-surprises-2014
- sources/ms-2019-03-15-bullish-bond-cyclone
- sources/ms-2019-03-25-rate-cuts-before-recessions
- sources/ms-2019-04-05-crossing-the-rubicon-government-bonds
- sources/ms-2019-05-17-tariffs-government-bonds
tags: []
title: Expectations Hypothesis of the Term Structure
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Expectations Hypothesis of the Term Structure

## Definition

The proposition that long-term rates reflect expected future short rates, so forward rates are unbiased predictors (pure version) or differ only by a constant risk premium; tested via a forward-rate slope coefficient of 1.

## Sources

- [[sources/bams-2003-risk-premia-term-structure-panel|Risk premia in the term structure of interest rates: a panel data approach]]

## Related Concepts

- [[concepts/factor-models|factor-models]]
- [[concepts/forward-rate|forward-rate]]
- [[concepts/mean-reversion|mean-reversion]]

## Added by credit-macro ingest (2026-06-09)

Now also discussed in: [[sources/ms-2013-12-13-top-10-rates-surprises-2014]], [[sources/ms-2019-03-15-bullish-bond-cyclone]], [[sources/ms-2019-03-25-rate-cuts-before-recessions]], [[sources/ms-2019-04-05-crossing-the-rubicon-government-bonds]], [[sources/ms-2019-05-17-tariffs-government-bonds]]