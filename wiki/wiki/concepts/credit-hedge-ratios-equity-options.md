---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/credit-hedge-ratios-equity-options
page_type: concept
related:
- concepts/cds-bond-basis
- concepts/corporate-bonds
- concepts/credit-spread-puzzle
sources:
- sources/avino-2024-hedging-credit-equity-options
tags: []
title: Credit Hedge Ratios with Equity Options
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Credit Hedge Ratios with Equity Options

## Definition

Theoretical sensitivities of bond credit spread changes to equity put option returns, derived by combining Merton and Geske via firm value V, used as weights in a replicating hedge portfolio.

## Sources

- [[sources/avino-2024-hedging-credit-equity-options|Contingent Claims and Hedging of Credit Risk with Equity Options]]

## Related Concepts

- [[concepts/cds-bond-basis|cds-bond-basis]]
- [[concepts/corporate-bonds|corporate-bonds]]
- [[concepts/credit-spread-puzzle|credit-spread-puzzle]]