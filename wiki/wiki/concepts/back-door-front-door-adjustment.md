---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/back-door-front-door-adjustment
page_type: concept
related:
- concepts/causal-inference
- concepts/double-machine-learning
revision_hash: sha256:032aa9fe032d9a0e88f0f630e53da3cc879099b97b37420263addb82dd82a748
revision_id: 1
sources:
- sources/pearl-2018-book-of-why
tags: []
title: Back-door and Front-door Adjustment
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Back-door and Front-door Adjustment

## Definition

Graphical criteria for identifying a set of variables to condition on (back-door) or an intermediate mechanism to exploit (front-door) so that a causal effect can be estimated from observational data.

## Sources

- [[sources/pearl-2018-book-of-why|The Book of Why: The New Science of Cause and Effect]]

## Related Concepts

- [[concepts/causal-inference|causal-inference]]
- [[concepts/double-machine-learning|double-machine-learning]]