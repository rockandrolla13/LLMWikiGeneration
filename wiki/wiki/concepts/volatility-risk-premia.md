---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/volatility-risk-premia
page_type: concept
related:
- concepts/carry-rolldown
- concepts/risk-premia
revision_id: 1
sources:
- sources/ms-2018-11-05-trading-risk-premia-emfx-virp
- sources/ms-2019-02-11-emfx-quants-lab-steady-path
tags: []
title: Volatility Risk Premia (VIRP)
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Volatility Risk Premia (VIRP)

## Definition

A systematic EMFX signal built on the gap between realized and implied currency volatility used to detect medium-term EM trends.

## Sources

- [[sources/ms-2018-11-05-trading-risk-premia-emfx-virp|Trading Risk Premia in EMFX (Part 2): A mixed strategy using Volatility Risk Premia (VIRP)]]
- [[sources/ms-2019-02-11-emfx-quants-lab-steady-path|EMFX Quant's Lab: Steady Path]]

## Related Concepts

- [[concepts/carry-rolldown|carry-rolldown]]
- [[concepts/risk-premia|risk-premia]]