---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/crisis-incidence-real-costs
page_type: concept
related:
- concepts/sovereign-debt-crisis
revision_id: 1
sources:
- sources/babecky-2013-leading-indicators-crisis-incidence
tags: []
title: Crisis Incidence and Real Costs
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Crisis Incidence and Real Costs

## Definition

A continuous measure of the real economic cost of a crisis, combining output loss, employment loss, and fiscal deficit, conditioned on the occurrence of a banking, currency, or debt crisis.

## Sources

- [[sources/babecky-2013-leading-indicators-crisis-incidence|Leading indicators of crisis incidence: Evidence from developed countries]]

## Related Concepts

- [[concepts/sovereign-debt-crisis|sovereign-debt-crisis]]