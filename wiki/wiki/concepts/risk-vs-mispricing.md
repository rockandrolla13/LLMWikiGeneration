---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/risk-vs-mispricing
page_type: concept
related:
- concepts/behavioral-finance
- concepts/factor-models
revision_id: 1
sources:
- sources/ilmanen-2022-investing-amid-low-expected-returns
tags: []
title: Risk-vs-Mispricing Debate
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Risk-vs-Mispricing Debate

## Definition

The framework distinguishing rational risk compensation ('bad returns in bad times') from behavioral mispricing as explanations for long-run premia, with crowding considerations.

## Sources

- [[sources/ilmanen-2022-investing-amid-low-expected-returns|Investing Amid Low Expected Returns: Making the Most When Markets Offer the Least]]

## Related Concepts

- [[concepts/behavioral-finance|behavioral-finance]]
- [[concepts/factor-models|factor-models]]