---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/vector-error-correction-model
page_type: concept
related:
- concepts/cointegration
- concepts/half-life-of-adjustment
- concepts/structural-vector-autoregression
sources:
- sources/duasa-2010-predicting-crisis-recovery
tags: []
title: Vector Error Correction Model
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Vector Error Correction Model

## Definition

A cointegrated VAR in which short-run dynamics of differenced I(1) variables are tied to a long-run equilibrium through error-correction terms; the speed-of-adjustment coefficient measures return speed.

## Sources

- [[sources/duasa-2010-predicting-crisis-recovery|Predicting the Global Crisis Recovery Period: Lessons from the 1997 Crisis]]

## Related Concepts

- [[concepts/cointegration|cointegration]]
- [[concepts/half-life-of-adjustment|half-life-of-adjustment]]
- [[concepts/structural-vector-autoregression|structural-vector-autoregression]]