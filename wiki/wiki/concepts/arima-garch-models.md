---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/arima-garch-models
page_type: concept
related:
- concepts/backtesting
- concepts/mean-reversion
revision_id: 1
sources:
- sources/halls-moore-advanced-algorithmic-trading
tags: []
title: ARIMA and GARCH Volatility Models
updated: '2026-06-20T01:03:51Z'
updated_by: creditmacro-batch
schema_version: 2
uuid: 37892063-3305-5f85-82cb-31ed527a431c
content_hash: sha256:c5333fac797bb2d069575baff63fffa4646a75e277ada9c79157902317cfdc8d
---

<!-- AUTHORED REGION START -->
# ARIMA and GARCH Volatility Models

## Definition

ARIMA models for the conditional mean combined with (G)ARCH models for time-varying conditional volatility, combined into an index trading strategy.

## Sources

- [[sources/halls-moore-advanced-algorithmic-trading|Advanced Algorithmic Trading]]

## Related Concepts

- [[concepts/backtesting|backtesting]]
- [[concepts/mean-reversion|mean-reversion]]
<!-- AUTHORED REGION END -->
