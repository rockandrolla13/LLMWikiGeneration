---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/input-output-analysis
page_type: concept
related: []
revision_id: 1
sources:
- sources/lukac-2026-economic-analysis-through-mathematics
tags: []
title: Input-Output Analysis
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Input-Output Analysis

## Definition

A linear-algebra-based macroeconomic modeling framework (Leontief) representing interdependencies between sectors as a system of linear equations, solved for gross output given final demand.

## Sources

- [[sources/lukac-2026-economic-analysis-through-mathematics|Economic Analysis Through Mathematics: Tools and Techniques for Decision Making]]