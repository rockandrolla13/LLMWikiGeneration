---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/market-integration-concordance-measure
page_type: concept
related:
- concepts/mean-reversion
- concepts/statistical-arbitrage
revision_id: 1
sources:
- sources/kapadia-2012-limited-arbitrage-equity-credit
tags: []
title: Market Integration Concordance Measure
updated: '2026-06-20T01:03:51Z'
updated_by: creditmacro-batch
schema_version: 2
uuid: 0e078915-8076-54d1-9a67-6eed71a3a2d5
content_hash: sha256:03173e4e51fb9471aec5f00f2ba6d6cb04e9a647fcc70fb004922b91ee677fb5
---

<!-- AUTHORED REGION START -->
# Market Integration Concordance Measure

## Definition

A nonparametric statistic (kappa) based on the concordance of stock-price and CDS-spread changes, equal to the Kendall rank correlation, used to detect Merton-model pricing discrepancies over short horizons.

## Sources

- [[sources/kapadia-2012-limited-arbitrage-equity-credit|Limited arbitrage between equity and credit markets]]

## Related Concepts

- [[concepts/mean-reversion|mean-reversion]]
- [[concepts/statistical-arbitrage|statistical-arbitrage]]
<!-- AUTHORED REGION END -->
