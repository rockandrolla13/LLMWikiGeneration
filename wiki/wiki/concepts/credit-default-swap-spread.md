---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/credit-default-swap-spread
page_type: concept
related:
- concepts/corporate-bonds
- concepts/credit-spread-puzzle
- concepts/liquidity-risk
revision_hash: sha256:e1120fae1d804a03c0a85efa23d81f76b7e26af177383b6e8dc0a2da741bc11b
revision_id: 1
sources:
- sources/kapadia-2012-limited-arbitrage-equity-credit
tags: []
title: Credit Default Swap Spread
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Credit Default Swap Spread

## Definition

The annualized cost of insurance against default on a firm's debt, used as the credit-market price input; here five-year senior unsecured CDS spreads from Markit.

## Sources

- [[sources/kapadia-2012-limited-arbitrage-equity-credit|Limited arbitrage between equity and credit markets]]

## Related Concepts

- [[concepts/corporate-bonds|corporate-bonds]]
- [[concepts/credit-spread-puzzle|credit-spread-puzzle]]
- [[concepts/liquidity-risk|liquidity-risk]]