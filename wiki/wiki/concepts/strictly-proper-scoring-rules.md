...
page_id: concepts/strictly-proper-scoring-rules
page_type: concept
related:
...
A scoring rule S(P,x) is proper if the expected score is maximized by quoting one's true predictive distribution, and strictly proper if that maximizer is unique, thereby encouraging honest probabilistic forecasts.
