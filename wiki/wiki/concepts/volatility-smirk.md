---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/volatility-smirk
page_type: concept
related: []
revision_hash: sha256:1bfa76768995ce33f992157394213ea194c605fc529f060083e23e9b3026176e
revision_id: 1
sources:
- sources/avino-2024-hedging-credit-equity-options
tags: []
title: Volatility Smirk as Informed-Trading Signal
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Volatility Smirk as Informed-Trading Signal

## Definition

The implied-volatility skew (OTM put IV minus ATM IV) used as a proxy for informed option trading; its changes predict the hedging-error gap between stocks and options.

## Sources

- [[sources/avino-2024-hedging-credit-equity-options|Contingent Claims and Hedging of Credit Risk with Equity Options]]