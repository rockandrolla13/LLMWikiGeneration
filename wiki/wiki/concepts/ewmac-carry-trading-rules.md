---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/ewmac-carry-trading-rules
page_type: concept
related:
- concepts/bond-momentum
- concepts/carry-rolldown
- concepts/mean-reversion
- concepts/trend-following
revision_id: 1
sources:
- sources/carver-2015-systematic-trading
tags: []
title: EWMAC and Carry Trading Rules
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# EWMAC and Carry Trading Rules

## Definition

Two generic example rules: EWMAC (exponentially weighted moving average crossover), a trend-following rule; and a carry rule based on expected return from holding an asset (a negative-skew strategy).

## Sources

- [[sources/carver-2015-systematic-trading|Systematic Trading: A unique new method for designing trading and investing systems]]

## Related Concepts

- [[concepts/bond-momentum|bond-momentum]]
- [[concepts/carry-rolldown|carry-rolldown]]
- [[concepts/mean-reversion|mean-reversion]]
- [[concepts/trend-following|trend-following]]