---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/instrumental-variables
page_type: concept
related:
- concepts/causal-inference
- concepts/local-average-treatment-effect
- concepts/regression-discontinuity
- concepts/structural-vector-autoregression
revision_hash: sha256:8dd4dadb1b48b9f6511f9939c61810e94c855fa195453a1443d64544e2177180
revision_id: 1
sources:
- sources/angrist-2009-mostly-harmless-econometrics
- sources/hernan-2020-causal-inference-what-if
- sources/pearl-2018-book-of-why
tags: []
title: Instrumental Variables
updated: '2026-06-20T01:03:51Z'
updated_by: creditmacro-batch
schema_version: 2
uuid: ab136def-b4bd-5aec-b152-c7303f750b84
content_hash: sha256:c66873caeabd4c7622ceb4b544bb18c3393c4747141b99369fa2bdc3c53cfa9f
---

<!-- AUTHORED REGION START -->
# Instrumental Variables

## Definition

A variable that affects the treatment but is otherwise independent of confounders and the outcome, used to recover a causal effect when direct adjustment for confounders is impossible.

## Sources

- [[sources/angrist-2009-mostly-harmless-econometrics|Mostly Harmless Econometrics: An Empiricist's Companion]]
- [[sources/hernan-2020-causal-inference-what-if|Causal Inference: What If]]
- [[sources/pearl-2018-book-of-why|The Book of Why: The New Science of Cause and Effect]]

## Related Concepts

- [[concepts/causal-inference|causal-inference]]
- [[concepts/local-average-treatment-effect|local-average-treatment-effect]]
- [[concepts/regression-discontinuity|regression-discontinuity]]
- [[concepts/structural-vector-autoregression|structural-vector-autoregression]]
<!-- AUTHORED REGION END -->
