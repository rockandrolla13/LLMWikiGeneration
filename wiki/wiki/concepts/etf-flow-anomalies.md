---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/etf-flow-anomalies
page_type: concept
related:
- concepts/algorithmic-trading
- concepts/etf-flows
revision_hash: sha256:17d727d9ca7bfcd781d901a087b43f79a6fd75fb0f8ee47cba183461e4078237
revision_id: 1
sources:
- sources/mercado-2015-taarss-flow-whisperer
tags: []
title: ETF Flow Anomalies (Create-to-Lend)
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# ETF Flow Anomalies (Create-to-Lend)

## Definition

Distortions in ETF flow data that do not reflect genuine directional demand, including lump-sum rebalancing and create-to-lend activity, which can produce misleading flow signals.

## Sources

- [[sources/mercado-2015-taarss-flow-whisperer|The Flow Whisperer: TAARSS says prefer a mix of bonds and equities in Q1]]

## Related Concepts

- [[concepts/algorithmic-trading|algorithmic-trading]]
- [[concepts/etf-flows|etf-flows]]