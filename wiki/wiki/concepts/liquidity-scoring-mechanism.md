---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/liquidity-scoring-mechanism
page_type: concept
related:
- concepts/liquidity-risk
revision_id: 1
sources:
- sources/ahmad-2014-alaph-liquid-macro-credit-fund
tags: []
title: Liquidity Scoring Mechanism
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Liquidity Scoring Mechanism

## Definition

A proprietary ranking methodology scoring tradable instruments on cost and time to exit using observable inputs - dealer-quote frequency, participating dealers, weekly notional and trade count, bid/offer spread, dispersion, and volatility.

## Sources

- [[sources/ahmad-2014-alaph-liquid-macro-credit-fund|Introduction to the Alaph Capital Liquid Macro Credit Fund]]

## Related Concepts

- [[concepts/liquidity-risk|liquidity-risk]]