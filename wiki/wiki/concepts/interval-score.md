---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/interval-score
page_type: concept
related:
- concepts/calibration
- concepts/prediction-intervals
- concepts/value-at-risk
revision_hash: sha256:032e13adcc52ae08b76dba4f7a307e34047fbd1fe249326432961569f4c1f602
revision_id: 1
sources:
- sources/gneiting-2007-strictly-proper-scoring-rules
tags: []
title: Interval Score
updated: '2026-06-20T01:03:51Z'
updated_by: creditmacro-batch
schema_version: 2
uuid: ff73f9b0-8885-58cf-b8ad-85e7de685f73
content_hash: sha256:77b567911d76105ac9e4d4440d5152b011f002cb056fe3ac7fbb6113bc3f0fbe
---

<!-- AUTHORED REGION START -->
# Interval Score

## Definition

A proper score for central prediction intervals that rewards narrow width and adds a penalty when the observation falls outside the interval, jointly assessing calibration and sharpness.

## Sources

- [[sources/gneiting-2007-strictly-proper-scoring-rules|Strictly Proper Scoring Rules, Prediction, and Estimation]]

## Related Concepts

- [[concepts/calibration|calibration]]
- [[concepts/prediction-intervals|prediction-intervals]]
- [[concepts/value-at-risk|value-at-risk]]
<!-- AUTHORED REGION END -->
