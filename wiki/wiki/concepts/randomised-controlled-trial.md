---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/randomised-controlled-trial
page_type: concept
related:
- concepts/causal-inference
- concepts/potential-outcomes
- concepts/quasi-experimental-design
revision_id: 1
sources:
- sources/hudson-2019-its-healthcare-reporting
tags: []
title: Randomised Controlled Trial
updated: '2026-06-20T01:03:51Z'
updated_by: creditmacro-batch
schema_version: 2
uuid: 3e53c071-8dc1-553c-bdeb-d2e3cb45b23f
content_hash: sha256:0b41c2f25f1a77483a96449e89a76f97cd7f123ed513f0bf6ef89c819941f8fd
---

<!-- AUTHORED REGION START -->
# Randomised Controlled Trial

## Definition

An experimental study design that uses random assignment of participants to intervention and control conditions, considered the gold standard for estimating the causal effect of a healthcare intervention.

## Sources

- [[sources/hudson-2019-its-healthcare-reporting|Methodology and reporting characteristics of studies using interrupted time series design in healthcare]]

## Related Concepts

- [[concepts/causal-inference|causal-inference]]
- [[concepts/potential-outcomes|potential-outcomes]]
- [[concepts/quasi-experimental-design|quasi-experimental-design]]
<!-- AUTHORED REGION END -->
