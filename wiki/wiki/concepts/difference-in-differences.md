---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/difference-in-differences
page_type: concept
related:
- concepts/causal-inference
- concepts/interrupted-time-series-design
- concepts/panel-vector-autoregression
- concepts/quasi-experimental-design
sources:
- sources/angrist-2009-mostly-harmless-econometrics
tags: []
title: Differences-in-Differences
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Differences-in-Differences

## Definition

A design using treated and control groups observed before and after an intervention; under common trends the double difference identifies the causal effect, via fixed-effects regression.

## Sources

- [[sources/angrist-2009-mostly-harmless-econometrics|Mostly Harmless Econometrics: An Empiricist's Companion]]

## Related Concepts

- [[concepts/causal-inference|causal-inference]]
- [[concepts/panel-vector-autoregression|panel-vector-autoregression]]

## Related (credit-macro ingest, 2026-06-09)

- [[concepts/interrupted-time-series-design|interrupted-time-series-design]]
- [[concepts/quasi-experimental-design|quasi-experimental-design]]