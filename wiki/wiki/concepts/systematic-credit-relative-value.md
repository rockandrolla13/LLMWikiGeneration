---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/systematic-credit-relative-value
page_type: concept
related:
- concepts/algorithmic-trading
- concepts/credit-relative-value
- concepts/factor-investing
- concepts/statistical-arbitrage
revision_hash: sha256:8ead875654f30a848377d8bc8c3d78bc4feab2c4e8772fe90fc5d41316457d2a
revision_id: 1
sources:
- sources/koukorinis-2024-xantium-business-plan
tags: []
title: Systematic Credit Relative Value
updated: '2026-06-20T01:03:51Z'
updated_by: creditmacro-batch
schema_version: 2
uuid: f6afc60f-522f-5506-80b4-8082a7510555
content_hash: sha256:665f05c6504661743dadbfd8190b8077b6cfeab3143b57e74927a16ee86660f5
---

<!-- AUTHORED REGION START -->
# Systematic Credit Relative Value

## Definition

A rules-based trading approach to liquid credit systematically evaluating relative value across cash bonds, CDS indices, ETFs, and derivatives, structuring market-neutral spread trades with asymmetric risk-reward.

## Sources

- [[sources/koukorinis-2024-xantium-business-plan|Business Plan Details for Xantium: Systematic Spread Fixed Income Trading]]

## Related Concepts

- [[concepts/algorithmic-trading|algorithmic-trading]]
- [[concepts/credit-relative-value|credit-relative-value]]
- [[concepts/factor-investing|factor-investing]]
- [[concepts/statistical-arbitrage|statistical-arbitrage]]
<!-- AUTHORED REGION END -->
