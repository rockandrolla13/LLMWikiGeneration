---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/law-of-active-management
page_type: concept
related:
- concepts/factor-investing
- concepts/mean-reversion
revision_hash: sha256:813481dd9da4ff0afb17771b0173eae0a84fcc138f9b8c5cf6b962c2940758eb
revision_id: 1
sources:
- sources/carver-2015-systematic-trading
tags: []
title: Law of Active Management
updated: '2026-06-20T01:03:51Z'
updated_by: creditmacro-batch
schema_version: 2
uuid: e39e0bfb-70ef-5878-b27e-787c0243a2fc
content_hash: sha256:c8c3bdba7ccd593c1d24e80fb17b576c4f0b1e0d845890836aee1195113faa77
---

<!-- AUTHORED REGION START -->
# Law of Active Management

## Definition

The principle that a strategy's Sharpe ratio scales with the square root of the number of independent bets per year, implying diversification across uncorrelated instruments raises achievable risk-adjusted return.

## Sources

- [[sources/carver-2015-systematic-trading|Systematic Trading: A unique new method for designing trading and investing systems]]

## Related Concepts

- [[concepts/factor-investing|factor-investing]]
- [[concepts/mean-reversion|mean-reversion]]
<!-- AUTHORED REGION END -->
