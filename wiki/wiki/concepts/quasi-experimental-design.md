---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/quasi-experimental-design
page_type: concept
related:
- concepts/causal-inference
- concepts/difference-in-differences
- concepts/potential-outcomes
- concepts/randomised-controlled-trial
revision_id: 1
sources:
- sources/hudson-2019-its-healthcare-reporting
tags: []
title: Quasi-Experimental Design
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Quasi-Experimental Design

## Definition

A study design used to estimate the effect of an intervention without random assignment, relying on pre- and post-intervention or comparison data when a randomised controlled trial is not feasible.

## Sources

- [[sources/hudson-2019-its-healthcare-reporting|Methodology and reporting characteristics of studies using interrupted time series design in healthcare]]

## Related Concepts

- [[concepts/causal-inference|causal-inference]]
- [[concepts/difference-in-differences|difference-in-differences]]
- [[concepts/potential-outcomes|potential-outcomes]]
- [[concepts/randomised-controlled-trial|randomised-controlled-trial]]