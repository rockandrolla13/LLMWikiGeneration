---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/correlation-vs-causation
page_type: concept
related:
- concepts/causal-inference
sources:
- sources/ellenberg-2014-how-not-to-be-wrong
tags: []
title: Correlation Is Not Causation
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Correlation Is Not Causation

## Definition

The principle that statistical association does not establish a causal link, explored through the lung-cancer/smoking debate and selection artifacts.

## Sources

- [[sources/ellenberg-2014-how-not-to-be-wrong|How Not to Be Wrong: The Power of Mathematical Thinking]]

## Related Concepts

- [[concepts/causal-inference|causal-inference]]