---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/phillips-curve
page_type: concept
related:
- concepts/great-moderation
sources:
- sources/schofield-2019-citi-macro-views
tags: []
title: Phillips Curve
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Phillips Curve

## Definition

The relationship between economic slack and inflation/wages; argued to be non-linear, with falling slack producing upward but uncertain pressure on wages and prices.

## Sources

- [[sources/schofield-2019-citi-macro-views|Citi Macro Views: Global Strategy and Macro Theme Book, Q1 2019]]

## Related Concepts

- [[concepts/great-moderation|great-moderation]]