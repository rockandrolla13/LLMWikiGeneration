---
title: In-Context Learning
page_id: concepts/in-context-learning
page_type: concept
revision_id: 1
created: '2026-05-17T16:44:06Z'
updated: '2026-06-20T01:03:51Z'
updated_by: wiki-batch-ai-engineering-2026-05-17
tags:
- ai-engineering
- prompt-engineering
- transformers
sources:
- sources/alammar-2024-hands-on-llm
- sources/huyen-2025-ai-engineering
related:
- concepts/prompt-engineering
- concepts/transformers
mind_map_priority: medium
schema_version: 2
uuid: 40bf554c-a190-5cfa-a2f1-c0a8e0052d38
content_hash: sha256:03d6108f047ccbea0c6941e1d6301b08a63f4707487a968945b985c29fcf1e33
---

<!-- AUTHORED REGION START -->
# In-Context Learning

## Definition

The ability of foundation models to perform new tasks at inference time by conditioning on instructions and examples provided in the prompt, without weight updates. Encompasses zero-shot and few-shot prompting as the dominant adaptation interface for closed models.

## Sources

- [[sources/alammar-2024-hands-on-llm|Hands-On Large Language Models]]
- [[sources/huyen-2025-ai-engineering|AI Engineering]]

## Related Concepts

- [[concepts/prompt-engineering]]
- [[concepts/transformers]]

<!-- AUTHORED REGION END -->
