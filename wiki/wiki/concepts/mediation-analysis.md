---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/mediation-analysis
page_type: concept
related:
- concepts/causal-inference
- concepts/four-way-effect-decomposition
revision_hash: sha256:45c7aa6f0eda00f3373f5e48100ad1683c7dda2f18fbc7ca06263c71f03b5b3a
revision_id: 1
sources:
- sources/pearl-2018-book-of-why
- sources/vanderweele-2015-explanation-causal-inference
tags: []
title: Mediation Analysis
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Mediation Analysis

## Definition

Decomposition of a causal effect into direct and indirect (mediator-routed) components to identify the mechanism through which a cause produces an effect.

## Sources

- [[sources/pearl-2018-book-of-why|The Book of Why: The New Science of Cause and Effect]]
- [[sources/vanderweele-2015-explanation-causal-inference|Explanation in Causal Inference: Methods for Mediation and Interaction]]

## Related Concepts

- [[concepts/causal-inference|causal-inference]]
- [[concepts/four-way-effect-decomposition|four-way-effect-decomposition]]