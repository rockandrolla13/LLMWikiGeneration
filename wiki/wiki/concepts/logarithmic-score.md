---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/logarithmic-score
page_type: concept
related:
- concepts/optimum-score-estimation
- concepts/strictly-proper-scoring-rules
- concepts/uncertainty-quantification
sources:
- sources/gneiting-2007-strictly-proper-scoring-rules
tags: []
title: Logarithmic Score
updated: '2026-06-09T12:00:00Z'
updated_by: creditmacro-batch
---

# Logarithmic Score

## Definition

The local strictly proper density score S(p,x)=log p(x), associated with Shannon entropy and Kullback-Leibler divergence; it underlies maximum likelihood estimation and log Bayes factors.

## Sources

- [[sources/gneiting-2007-strictly-proper-scoring-rules|Strictly Proper Scoring Rules, Prediction, and Estimation]]

## Related Concepts

- [[concepts/optimum-score-estimation|optimum-score-estimation]]
- [[concepts/strictly-proper-scoring-rules|strictly-proper-scoring-rules]]
- [[concepts/uncertainty-quantification|uncertainty-quantification]]