---
abstraction_level: intermediate
concept_type: technique
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: concepts/dsge-threshold-bvar-counterfactual-analysis
page_type: concept
related:
- concepts/spillover-effect
- concepts/structural-vector-autoregression
revision_id: 1
sources:
- sources/rostagno-2021-ecb-monetary-policy-crisis
tags: []
title: DSGE and Threshold BVAR Counterfactual Analysis
updated: '2026-06-20T01:03:51Z'
updated_by: creditmacro-batch
schema_version: 2
uuid: 313090e8-75aa-5791-8865-dd4f9683a641
content_hash: sha256:15e49c3bd90cc08f610ca35e14698f698016a9dec15e2b69c8c5343bb3abb6b0
---

<!-- AUTHORED REGION START -->
# DSGE and Threshold BVAR Counterfactual Analysis

## Definition

The book's empirical toolkit: an estimated DSGE model, a two-regime threshold Bayesian VAR, Taylor-rule estimates, and counterfactual yield-curve simulations to attribute outcomes and quantify policy effects.

## Sources

- [[sources/rostagno-2021-ecb-monetary-policy-crisis|Monetary Policy in Times of Crisis: A Tale of Two Decades of the European Central Bank]]

## Related Concepts

- [[concepts/spillover-effect|spillover-effect]]
- [[concepts/structural-vector-autoregression|structural-vector-autoregression]]
<!-- AUTHORED REGION END -->
