---
authors:
- Alexander Wojt
- Koichi Sugisaki
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: sources/ms-2016-03-22-xccy-basis-primer
page_type: source
publication_date: '2016'
publication_venue: Morgan Stanley Research
related:
- concepts/cross-currency-basis-swap
- concepts/forward-rate
- concepts/liquidity-risk
- entities/alexander-wojt
- entities/koichi-sugisaki
- entities/morgan-stanley
revision_hash: sha256:f2b9f9e7c7aaaa6e70c099f1cb7bce7d80142f909c2a77b552eae42bb1736abe
revision_id: 1
source_hash: sha256:7b0d1dc95865649288fe0f240a780d02f3ac7f95e1fb4e9aa6571758c31edc56
source_path: raw/creditmacro/Morgan Stanley - Global Interest Rate Strategy — Note
  Information as of Match 22, 2016, unless otherwise indicated. - 2016.md
source_type: article
sources: []
tags: []
title: 'Global Interest Rate Strategy: XCCY Basis Primer'
updated: '2026-06-20T01:03:51Z'
updated_by: op_0e01ecc9062b
schema_version: 2
uuid: 3adff7d3-8607-5a43-a80f-202cdb0f2443
content_hash: sha256:d62244e0246651b213b1fed546c663ec4f0e325171a20921e2727afb62d51006
---

<!-- AUTHORED REGION START -->
# Global Interest Rate Strategy: XCCY Basis Primer

**Authors:** Alexander Wojt, Koichi Sugisaki · **Year:** 2016 · **Venue:** Morgan Stanley Research · **Type:** report

## Summary

A primer on cross-currency (XCCY) basis swaps covering their mechanics, drivers, and risks. It defines an XCCY basis swap as a floating/floating swap where two parties simultaneously borrow and lend in two different currencies, with the basis quoted on the non-USD leg and reflecting the relative price of liquidity across currencies. The report explains quotation conventions, cash flows, and basis calculation, then examines drivers such as foreign-currency issuance and asset purchases, FX, rates, liquidity, and regulation, with additional material on decomposing the basis and comparing FX forwards to XCCY swaps.

## Key Claims

1. An XCCY basis swap is a floating/floating swap in which parties simultaneously borrow and lend in two different currencies, with the basis quoted on the non-USD leg.
2. The XCCY basis reflects the relative price of liquidity in one currency versus another.
3. Key drivers and risks of the basis include foreign-currency issuance and asset purchases, FX, rates, liquidity, and regulation.

## Concepts

- [[concepts/cross-currency-basis-swap|Cross-Currency Basis Swap]]
- [[concepts/liquidity-risk|Liquidity Risk]]
- [[concepts/forward-rate|Forward Rate]]

## Entities

- [[entities/morgan-stanley|Morgan Stanley]]
- [[entities/alexander-wojt|Alexander Wojt]]
- [[entities/koichi-sugisaki|Koichi Sugisaki]]

## Source

- **Path:** `raw/creditmacro/Morgan Stanley - Global Interest Rate Strategy — Note Information as of Match 22, 2016, unless otherwise indicated. - 2016.md`
- **Type:** article
- **Hash:** `sha256:7b0d1dc9586564928...`
<!-- AUTHORED REGION END -->
