---
authors:
- Phanikiran Naraparaju
- Sheena Shah
- Wanting Low
- Vishwas Patkar
- Kelcie Gerson
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: sources/ms-2019-06-28-fx-volatility-playbook
page_type: source
publication_date: '2019'
publication_venue: Morgan Stanley Research
related:
- concepts/carry-rolldown
- concepts/cross-asset-rotation
- concepts/implied-volatility-skew
- concepts/quantitative-easing
- entities/kelcie-gerson
- entities/morgan-stanley
- entities/phanikiran-naraparaju
- entities/sheena-shah
- entities/vishwas-patkar
- entities/wanting-low
revision_hash: sha256:b0dad6ff252b48316577ee0d8749b8897777bbd32c3bbc36ee6445a877bc7e93
revision_id: 1
source_hash: sha256:d48e5c13e435e351ee91197e9c9b90dc0f8d857f00132d5f17ccf928ea900176
source_path: raw/creditmacro/Morgan Stanley - Global Derivatives Strategy — Global
  Volatility Playbook What’s Wrong with FX Volatility - 2019-06-28.md
source_type: article
sources: []
tags: []
title: 'Global Volatility Playbook: What''s Wrong with FX Volatility'
updated: '2026-06-09T12:00:00Z'
updated_by: op_690b7614875d
---

# Global Volatility Playbook: What's Wrong with FX Volatility

**Authors:** Phanikiran Naraparaju, Sheena Shah, Wanting Low, Vishwas Patkar, Kelcie Gerson · **Year:** 2019 · **Venue:** Morgan Stanley Research · **Type:** report

## Summary

Morgan Stanley's monthly cross-asset volatility playbook for June 2019. Commodity vols (gold, Brent) and front-end rate vols spiked as the Fed signalled an easing bias, while G10 FX volatility (notably KRW and EUR) stayed stubbornly low and skew flattened across asset classes on a perceived Fed 'put'. The team argues easier policy is a pause rather than a new prolonged low-vol regime and that FX vols can still rise if the USD begins trending. Key trades include FX and equity worst-of-put correlation baskets, conditional JPY bull-flatteners, and a credit vol decompression trade buying HY puts funded with CDX IG puts.

## Key Claims

1. A spike in commodity volatility (gold and Brent) was the standout move for June 2019, even as equity and credit vols eased on a perceived Fed 'put'.
2. G10 FX volatility remained low because central banks turned proactively dovish in unison, preventing a strong USD trend; FX vols can still rise if the USD starts trending.
3. The 1-year 1-sigma tail for US 2yr rates is already priced at roughly 0.8% versus spot of 1.8%, reflecting widespread rate-cut signalling.
4. Easier policy is viewed as a pause in the volatility cycle rather than the start of a new prolonged low-vol backdrop, given expensive valuations and less slack than prior QE episodes.

## Concepts

- [[concepts/implied-volatility-skew|Implied volatility skew]]
- [[concepts/quantitative-easing|Quantitative easing]]
- [[concepts/carry-rolldown|Carry and rolldown]]
- [[concepts/cross-asset-rotation|Cross-asset rotation]]

## Entities

- [[entities/morgan-stanley|Morgan Stanley]]
- [[entities/phanikiran-naraparaju|Phanikiran Naraparaju]]
- [[entities/sheena-shah|Sheena Shah]]
- [[entities/wanting-low|Wanting Low]]
- [[entities/vishwas-patkar|Vishwas Patkar]]
- [[entities/kelcie-gerson|Kelcie Gerson]]

## Source

- **Path:** `raw/creditmacro/Morgan Stanley - Global Derivatives Strategy — Global Volatility Playbook What’s Wrong with FX Volatility - 2019-06-28.md`
- **Type:** article
- **Hash:** `sha256:d48e5c13e435e351e...`