---
authors:
- Robert J Brown
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: sources/ms-2019-02-13-eurex-futures-rolls
page_type: source
publication_date: '2019'
publication_venue: Morgan Stanley Research
related:
- concepts/carry-rolldown
- concepts/forward-rate
- concepts/government-bond-spreads
- entities/morgan-stanley
- entities/robert-j-brown
revision_hash: sha256:c95764bbd2edca533049ab24f5c5a6df1456a2c2517df210a5f6ff3df23aea15
revision_id: 1
source_hash: sha256:49369714d3875eea45db23b1949237ace88bfda3367de7f551f7f2c0c15d36f3
source_path: raw/creditmacro/Morgan Stanley - Global Bond Futures Strategy — Correction
  Global Futures Rolls Report Eurex Futures Rolls - Mar 19 Jun 19 Roll - 2019-02-13.md
source_type: article
sources: []
tags: []
title: 'Correction: Global Futures Rolls Report: Eurex Futures Rolls - Mar 19 / Jun
  19 Roll'
updated: '2026-06-20T01:03:51Z'
updated_by: op_29f443fac943
schema_version: 2
uuid: fa50cafa-45c7-51b5-b225-b294d4ea3c47
content_hash: sha256:21599dcae8db5b2ac701abeec2b53e34f1a437c5a8d3b078a9fa9d30a25470dc
---

<!-- AUTHORED REGION START -->
# Correction: Global Futures Rolls Report: Eurex Futures Rolls - Mar 19 / Jun 19 Roll

**Authors:** Robert J Brown · **Year:** 2019 · **Venue:** Morgan Stanley Research · **Type:** report

## Summary

A Global Bond Futures Strategy note presenting views on Eurex futures contract rolls for the March-to-June 2019 cycle across Schatz, Bobl, Bund, Buxl, and BTP contracts. It analyzes fundamental drivers of each roll, investor positioning, delivery optionality, and relative value versus model-implied fair value. The note is a republished correction that revised the fair-value estimate of the roll in Exhibit 8 and accompanying text.

## Key Claims

1. The Schatz roll is rated neutral as the new March 2021 CTD issuance complicates fair-value estimation and obscures any directional signal.
2. Bobl and Buxl rolls are mildly bearish given long positioning bias that could create cheapening pressure.
3. The Bund roll is neutral with no substantial optionality and no significant positioning bias.
4. The BTP roll is mildly bullish, supported by historical richening into delivery and a slight bearish positioning bias.

## Concepts

- [[concepts/carry-rolldown|Carry and Rolldown]]
- [[concepts/forward-rate|Forward Rate]]
- [[concepts/government-bond-spreads|Government Bond Spreads]]

## Entities

- [[entities/robert-j-brown|Robert J Brown]]
- [[entities/morgan-stanley|Morgan Stanley]]

## Source

- **Path:** `raw/creditmacro/Morgan Stanley - Global Bond Futures Strategy — Correction Global Futures Rolls Report Eurex Futures Rolls - Mar 19 Jun 19 Roll - 2019-02-13.md`
- **Type:** article
- **Hash:** `sha256:49369714d3875eea4...`
<!-- AUTHORED REGION END -->
