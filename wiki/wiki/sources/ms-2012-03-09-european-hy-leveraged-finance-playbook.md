---
authors:
- Andrew Sheets
- Phanikiran Naraparaju
- Serena Tang
- Jonathan Graber
- Joerund Holterud Aarsnes
created: '2026-06-09T12:00:00Z'
mind_map_category: null
mind_map_priority: medium
page_id: sources/ms-2012-03-09-european-hy-leveraged-finance-playbook
page_type: source
publication_date: '2012'
publication_venue: Morgan Stanley Research
related:
- concepts/cds-bond-basis
- concepts/default-rates
- concepts/distressed-exchange
- entities/andrew-sheets
- entities/moodys
- entities/morgan-stanley
- entities/phanikiran-naraparaju
- entities/serena-tang
revision_hash: sha256:f8196b49913afc117a622c0832aadfca16059df0a0d52b434cd57258ec6904c7
revision_id: 1
source_hash: sha256:5ccccf84fcaa95ebcda0a6fb180d2de7cad983e2134e5efed844a1edec436b77
source_path: raw/creditmacro/Morgan Stanley - European High Yield Strategy Monthly
  Leveraged Finance Playbook — Returns - 2012-03-09.md
source_type: article
sources: []
tags: []
title: European High Yield Strategy Monthly Leveraged Finance Playbook — Returns
updated: '2026-06-09T12:00:00Z'
updated_by: op_250ae2fc3e19
---

# European High Yield Strategy Monthly Leveraged Finance Playbook — Returns

**Authors:** Andrew Sheets, Phanikiran Naraparaju, Serena Tang, Jonathan Graber, Joerund Holterud Aarsnes · **Year:** 2012 · **Venue:** Morgan Stanley Research · **Type:** report

## Summary

Monthly leveraged finance playbook for European high yield, reporting that the asset class returned 1.8% over the past month, led by beta, with CCCs returning 4.2% and industrials outperforming other non-financials. The trailing 12-month European speculative-grade default rate declined to 2.7% at end-January per Moody's. The report tracks index performance, cash and CDS indices, single-name out/underperformers, leveraged finance issuance and maturities, cash-CDS basis outliers, and distressed bonds.

## Key Claims

1. European high yield returned 1.8% over the past month, led by beta, with CCCs returning 4.2%.
2. The trailing 12-month European speculative-grade default rate declined to 2.7% at end-January per Moody's data.
3. In CDS, CWLN and MESSA outperformed while TKAGR underperformed; in cash, CMACG and PAJFP outperformed while IDEABB and CEDC underperformed.

## Concepts

- [[concepts/default-rates|Default Rates]]
- [[concepts/cds-bond-basis|CDS-Bond Basis]]
- [[concepts/distressed-exchange|Distressed Exchange]]

## Entities

- [[entities/morgan-stanley|Morgan Stanley]]
- [[entities/andrew-sheets|Andrew Sheets]]
- [[entities/phanikiran-naraparaju|Phanikiran Naraparaju]]
- [[entities/serena-tang|Serena Tang]]
- [[entities/moodys|Moody's]]

## Source

- **Path:** `raw/creditmacro/Morgan Stanley - European High Yield Strategy Monthly Leveraged Finance Playbook — Returns - 2012-03-09.md`
- **Type:** article
- **Hash:** `sha256:5ccccf84fcaa95ebc...`