---
title: Apache Kafka
page_id: entities/apache-kafka
page_type: entity
entity_type: tool
revision_id: 1
created: '2026-05-17T16:44:06Z'
updated: '2026-05-17T16:44:06Z'
updated_by: wiki-batch-ai-engineering-2026-05-17
tags:
- ai-engineering
- tool
sources:
- sources/barrasa-2023-building-knowledge-graphs
- sources/huyen-2022-designing-ml-systems
mind_map_priority: medium
affiliation: Confluent
---

# Apache Kafka

**Role:** tool

**Affiliation:** Confluent

## Notes

Real-time transport repeatedly referenced for moving fresh data from applications to ML systems before warehouse deposit; underpins the streaming-first architecture Huyen advocates.

## Sources in This Wiki

- [[sources/barrasa-2023-building-knowledge-graphs|Building Knowledge Graphs]]
- [[sources/huyen-2022-designing-ml-systems|Designing Machine Learning Systems]]
