---
title: LangChain
page_id: entities/langchain
page_type: entity
entity_type: tool
revision_id: 1
created: '2026-05-17T16:44:06Z'
updated: '2026-06-20T01:03:51Z'
updated_by: wiki-batch-ai-engineering-2026-05-17
tags:
- ai-engineering
- tool
sources:
- sources/alammar-2024-hands-on-llm
- sources/boonstra-2024-google-prompt-engineering
- sources/bratanic-2025-essential-graphrag
- sources/caelen-2023-developing-apps-gpt4
- sources/huyen-2025-ai-engineering
- sources/oshin-2025-learning-langchain
- sources/thomas-2025-ai-value-creators
mind_map_priority: high
affiliation: LangChain, Inc.
revision_hash: sha256:82049afdf76abc15
schema_version: 2
uuid: c6b7f329-ebec-5388-a78f-d9f4b9736b2b
content_hash: sha256:ae2354a2fbd174ada1d7a6e1a0e9562708f61d8a95d446ed999026533a515e25
---

<!-- AUTHORED REGION START -->
# LangChain

**Role:** tool

**Affiliation:** LangChain, Inc.

## Notes

Framework for chaining LLM calls and tools, used in the book as an example of today's ad-hoc orchestration that 'generative computing' aspires to replace with a proper runtime.

## Sources in This Wiki

- [[sources/alammar-2024-hands-on-llm|Hands-On Large Language Models]]
- [[sources/boonstra-2024-google-prompt-engineering|Prompt Engineering]]
- [[sources/bratanic-2025-essential-graphrag|Essential GraphRAG]]
- [[sources/caelen-2023-developing-apps-gpt4|Developing Apps with GPT-4 and ChatGPT]]
- [[sources/huyen-2025-ai-engineering|AI Engineering]]
- [[sources/oshin-2025-learning-langchain|Learning LangChain]]
- [[sources/thomas-2025-ai-value-creators|AI Value Creators]]

<!-- AUTHORED REGION END -->
