---
title: Neo4j
page_id: entities/neo4j
page_type: entity
entity_type: tool
revision_id: 1
created: '2026-05-17T16:44:06Z'
updated: '2026-05-17T16:44:06Z'
updated_by: wiki-batch-ai-engineering-2026-05-17
tags:
- ai-engineering
- tool
sources:
- sources/barrasa-2023-building-knowledge-graphs
- sources/bratanic-2025-essential-graphrag
mind_map_priority: medium
revision_hash: sha256:9aaa7c2800a72960
---

# Neo4j

**Role:** tool

## Notes

Property graph database used throughout the book for all code examples; provides the Cypher query language, APOC library, Graph Data Science library, Bloom visualisation, and Aura/Sandbox cloud offerings.

## Sources in This Wiki

- [[sources/barrasa-2023-building-knowledge-graphs|Building Knowledge Graphs]]
- [[sources/bratanic-2025-essential-graphrag|Essential GraphRAG]]
