---
title: ChatGPT
page_id: entities/chatgpt
page_type: entity
entity_type: tool
revision_id: 1
created: '2026-05-17T16:44:06Z'
updated: '2026-05-17T16:44:06Z'
updated_by: wiki-batch-ai-engineering-2026-05-17
tags:
- ai-engineering
- tool
sources:
- sources/taulli-2024-ai-assisted-programming
- sources/wilson-2024-llm-security-playbook
mind_map_priority: medium
affiliation: OpenAI
revision_hash: sha256:73923898b079f60d
---

# ChatGPT

**Role:** tool

**Affiliation:** OpenAI

## Notes

General-purpose chat LLM that anchors Chapter 6 — used for regex, starter code, README generation, bash commands, GitHub Actions, and as a coding tutor; the book also covers GPT-4, Custom Instructions, Browse with Bing, Plugins, and GPTs.

## Sources in This Wiki

- [[sources/taulli-2024-ai-assisted-programming|AI-Assisted Programming]]
- [[sources/wilson-2024-llm-security-playbook|The Developer's Playbook for Large Language Model Security]]
