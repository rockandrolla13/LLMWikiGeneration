---
title: Luca Bosatta
page_id: entities/luca-bosatta
page_type: entity
entity_type: person
created: 2026-05-05T23:25:00Z
updated: 2026-05-05T23:25:00Z
tags: [creditETF, risk-modelling, credit-curves]
sources: [sources/ubs-2012-next-gen-credit-curves]
related: [entities/ubs-delta, entities/lindsey-matthews, concepts/hazard-rate-curve]
---

# Luca Bosatta

**Luca Bosatta** is Managing Director and Head of Risk Modelling at [[entities/ubs-delta|UBS Delta]].

## Research Contributions

### Next-Generation Credit Curves (2012)

Co-authored with [[entities/lindsey-matthews|Lindsey Matthews]], presented UBS Delta's D-Curves methodology:

- Unified hazard-rate curves combining bonds and CDS
- Joint estimation across multiple currencies
- Market surfaces reducing artificial rating-migration jumps
- Bond-implied CDS derivation for issuers without traded CDS

## Affiliation

- UBS Delta - Head of Risk Modelling (as of 2012)

## Publications in This Wiki

- [[sources/ubs-2012-next-gen-credit-curves|Next-Generation Credit Curves]]
